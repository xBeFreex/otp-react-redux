...
function areSameOriginAndDestination(search1, search2) {
  return (
    search1.query.from.name === search2.query.from.name &&
    search1.query.to.name === search2.query.to.name
  )
}

/**
 * Remove old entries from a search with the same origin/destination places.
 */
export function tidyRecentSearches(searches) {
  const sortedSearches = [...searches].sort(
    // When the entries are loaded, they are in ascending time order (most recent last)
    // but we want to display the most recents first.
    (entry1, entry2) => entry2.timestamp - entry1.timestamp
  )

  const result = []
  for (let i = 0; i < sortedSearches.length; i++) {
    const thisSearch = sortedSearches[i]
    if (i === 0) {
      result.push(thisSearch)
    } else {
      let isExistingSearch = false
      for (let j = 0; j < i; j++) {
        // Add the search if it has not appeared before.
        const previousSearch = sortedSearches[j]
        if (areSameOriginAndDestination(thisSearch, previousSearch)) {
