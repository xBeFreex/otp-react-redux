...
class NarrativeItineraries extends Component {
  ...
  itineraryMenuRef = React.createRef()
  ...
  componentDidUpdate(prevProps) {
    // Scroll the itinerary list/detail container back to the top whenever
    // switching between the results list and the expanded itinerary detail
    // view, so the detail view doesn't open still scrolled down.
    if (prevProps.itineraryIsExpanded !== this.props.itineraryIsExpanded) {
      // Reset scroll on every scrollable ancestor (e.g. the floating sidebar
      // card and any inner scroll containers), not just #itinerary-menu,
      // so the newly-shown view is never left scrolled out of sight.
      let el = this.itineraryMenuRef.current
      while (el) {
            ...
            Object.keys(groupedMergedItineraries.multi).map((mode) => {
              return (
                <S.ModeResultContainer
                  className="mode-results-container"
                  key={mode}
                >
                  ...
                  <h3>{mode}</h3>
                  <ListContainer className="mode-result-list">
          ...
          {groupItineraries &&
            ...
            Object.keys(groupedMergedItineraries.single).length > 0 && (
              <S.ModeResultContainer>
                ...
                <S.SingleModeRowContainer className="single-mode-container">
